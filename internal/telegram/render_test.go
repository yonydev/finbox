...
func TestCardCategoryLine(t *testing.T) {
	if c := Card("a3f2c9d1", sampleValidated(), false); !strings.Contains(c, "🏷 "+messages.NoCategory+"\n") {
		t.Errorf("uncategorized card missing the 🏷 line:\n%s", c)
	}
	v := sampleValidated()
	v.Category, v.CategorySource = "educacion", "human"
	if c := Card("a3f2c9d1", v, false); !strings.Contains(c, "🏷 educación · "+messages.CategoryByYou+"\n") {
		t.Errorf("labeled card missing the 🏷 line:\n%s", c)
	}
}

func TestMonthSummaryOrdersUncategorizedLast(t *testing.T) {
	...
	want := "<b>2026-08</b>: $534.00 MXN + $45.50 USD · 4 gastos\n" +
		"🏷 súper · $50.00 MXN · 1\n🏷 sin categoría · $484.00 MXN · 2\n🏷 sin categoría · $45.50 USD · 1"
	if got != want {
